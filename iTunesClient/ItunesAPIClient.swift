...
//    let downloader = JSONDownloader()
//    
//    // Creaing 3 methods of API 
//    // Starting with the generic serach
//    
//    func searchForArtists(withTerm term: String, completion: @escaping ([Artist], ItunesError?) -> Void) {
//        let endpoint = Itunes.search(term: term, media: .music(entity: .musicArtist, attribute: .artistTerm))
//        
//        // Instead of creating 3 JSON we are doing in 1
//        
//        performRequest(with: endpoint) { results, error in
//            guard let results = results else {
//                completion([], error)
...
//        }
//        
//        // Creating a task not starting it
////        let task = downloader.jsonTask(with: endpoint.request) { json, error in
////            // Defining a closure, what we want to do with the JSON data that we get back in the event that it failed the error object
////            // Jumping on the mein Thread here
////            DispatchQueue.main.async {
////                guard let json = json else {
////                    completion([], error)
////                    return
////                }
////                
////                // Parsing the data and converting into our model objects
////                guard let results = json["results"] as? [[String: Any]] else {
